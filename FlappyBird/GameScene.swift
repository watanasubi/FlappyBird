...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        //各種スライドを生成する処置をメソッドに分割
        setupGround()
        setupCloud()
        setupWall()
        setupBird()
        setupScoreLabel()
        setupItem() //アイテム　以下にセットアップ記載
    ...
    func setupItem() {
        ...
        // アイテムの画像を読み込む
        let itemTexture = SKTexture(imageNamed: "mask")
...
func setupWall(){
    ...
    //鳥の画像サイズを取得
    let birdSize = SKTexture(imageNamed: "bira_a").size()
    
    //鳥が通り抜ける隙間の長さを鳥のサイズの３倍とする
    let slit_length = birdSize.height * 3
    
    //隙間位置の上下の振れ幅を鳥サイズの２.２倍とする
    let random_y_range = birdSize.height * 2.2
    
    //下の壁のY軸下弦位置（中央位置から下方向の最大振れ幅で下の壁を表示する位置）を計算
